...
public class Test02 {
    ...
    public static String solution(String play_time, String adv_time, String[] logs) throws ParseException {
        String answer;
        long playSeconds = convertTimeToSeconds(play_time); // 동영상 재생 시간(초로 변환)
        long advSeconds = convertTimeToSeconds(adv_time); // 광고 재생 시간 (초로 변환)
        ...
        for (String log : logs) {
            ...
            long logStartSeconds = convertTimeToSeconds(logStartTime);

            // 로그 종료 시간
            String logEndTime = logString[1];
            long logEndSeconds = convertTimeToSeconds(logEndTime);

            for (long i = logStartSeconds; i < logEndSeconds; i++) {
                countMap.put(i, countMap.getOrDefault(i, 0) + 1);
            }
        }

        long max = Long.MIN_VALUE;
        long answerSeconds = 0;
        long start = 0;
        long end = advSeconds;
        long totalSeconds = 0;
        while (end <= playSeconds) {
            if (start == 0) {
                for (long i = start; i < end; i++) {
                    totalSeconds += countMap.getOrDefault(i, 0);
                }
            } else {
                totalSeconds = totalSeconds - countMap.getOrDefault(start - 1, 0) + countMap.getOrDefault(end - 1, 0);
            }
